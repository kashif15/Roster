...
// Utility to generate random 6-digit ID and random email
...
app.post('/upload', verifyToken, isAdmin, excelUpload.single('file'), async (req, res) => {
    ...
    try {
        ...
        for (let i = 2; i < rows.length; i++) {
            ...
            // If employee doesn't exist, create a new one with random ID and email
            if (!employee) {
            ...
            for (let j = 1; j < row.length; j++) {
                const shiftType = row[j]?.trim();
                const dateStr = rows[0][j];  // Get date from the first row
                const date = XLSX.SSF.parse_date_code(dateStr);  // Parse date from Excel
...
app.put('/update',  verifyToken, isAdmin, imageUpload.single('image'), async (req, res) => {
    ...
    try {
        ...
        // Update the employee details with provided data
        if (employeeId) {
        ...
        if (req.file) {
            const imagePath = `/uploads/${req.file.filename}`;  // Save the relative path to the uploaded image
